...
public class FiniteStateMachineIntake {
	...
	// Vision member
	private final VisionIntakeYellowLocator vision;
	...
	//Constructor
	public FiniteStateMachineIntake(RobotHardware robot, GamepadEx gamepad_1, GamepadEx gamepad_2, FiniteStateMachineDeposit depositArmDrive, VisionIntakeYellowLocator vision) {
	...
	public void IntakeArmLoop() {
		...
		switch (intakeState) {
			...
			case INTAKE_PICK:
				if (intakeTimer.seconds() > RobotActionConfig.intakeSlideExtendTime) {
					...
					if ((gamepad_1.getButton(DPAD_RIGHT) || gamepad_2.getButton(DPAD_RIGHT)) && isButtonDebounced()) {
						if (RobotActionConfig.visionIntakeEnabled) {
							visionTimer.reset();
							intakeState = INTAKESTATE.INTAKE_VISION_ALIGN;
						} else {
			...
			case INTAKE_VISION_ALIGN:
				// Use camera to center yellow sample and adjust slide extension
				VisionIntakeYellowLocator.Detection det = vision != null ? vision.getBestDetection() : null;
				if (det == null || !det.hasTarget) {
					if (visionTimer.seconds() > RobotActionConfig.visionMaxAlignTime) {
						// timeout -> fallback to manual retract
						robot.intakeArmServo.setPosition(RobotActionConfig.intake_Arm_Grab);
						intakeTimer.reset();
						intakeState = INTAKESTATE.INTAKE_TRANS_READY;
					}
				...
				// Horizontal alignment using turret
				double imageCenterX = VisionIntakeYellowLocator.cameraWidth / 2.0;
				double errorX = (det.centerX - imageCenterX) / imageCenterX; // -1..1
				double turretPos = robot.intakeTurretServo.getPosition();
				if (Math.abs(errorX) > RobotActionConfig.visionTurretDeadband) {
					// Note: sign may need flipping depending on servo install
					double step = Range.clip(-RobotActionConfig.visionTurretKp * errorX, -RobotActionConfig.visionMaxTurretStep, RobotActionConfig.visionMaxTurretStep);
					turretPos = Range.clip(turretPos + step, 0.0, 1.0);
					robot.intakeTurretServo.setPosition(turretPos);
				}

				// Adjust slide extension based on area (bigger area -> closer -> retract a bit)
				double area = det.area;
				double extMin = RobotActionConfig.intake_Slide_Retract;
				double extMax = RobotActionConfig.intake_Slide_Extension;
				double mapped = extMax - (Range.clip((area - RobotActionConfig.visionMinAreaForMapping) / Math.max(1.0, (RobotActionConfig.visionTargetArea - RobotActionConfig.visionMinAreaForMapping)), 0.0, 1.0) * (extMax - extMin));
				robot.intakeLeftSlideServo.setPosition(Range.clip(mapped, extMin, extMax));
				robot.intakeRightSlideServo.setPosition(Range.clip(mapped, extMin, extMax));

				// If centered and big enough, drop arm and close claw -> proceed to transfer
				if (Math.abs(errorX) <= RobotActionConfig.visionCompleteDeadband && area >= RobotActionConfig.visionPickupArea) {
					robot.intakeArmServo.setPosition(RobotActionConfig.intake_Arm_Grab);
