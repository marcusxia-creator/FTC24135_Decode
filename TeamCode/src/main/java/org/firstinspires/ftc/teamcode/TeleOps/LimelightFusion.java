...
public class LimelightFusion {
    ...
    public LimelightFusion(RobotHardware robot, Turret turret) {
        this.robot = robot;
        this.turret = turret;
    ...
    public Output normalizedPose2D(DistanceUnit distanceUnit) {
        ...
        if (stats.filtered) {
            filteredPose = new Pose2D(DistanceUnit.MM, stats.meanRxMm, stats.meanRyMm, AngleUnit.DEGREES, robotVisionPose2D.getHeading(AngleUnit.DEGREES));
        }
        else {
            filteredPose = robotVisionPose2D;
        }

        Pose2D fusedPose = fusePose(robot.pinpoint.getPosition(), filteredPose, alpha);

        boolean didRest = false;
        double error = Math.hypot(filteredPose.getX(DistanceUnit.MM) - robot.pinpoint.getPosX(DistanceUnit.MM),
                filteredPose.getY(DistanceUnit.MM) - robot.pinpoint.getPosY(DistanceUnit.MM));
        ...
        return new Output(finalPose, robotVisionPose2D, robot.pinpoint.getPosition(), filteredPose,
                tx, ty, quality, alpha,
    ...
    private Pose2D normalizedRobotPose(Pose3D visionPose3D) {
        ...
        return new Pose2D(DistanceUnit.MM, (visionPose3D.getPosition().x * conversionFactor + xOffSet + turretXOffSet), (visionPose3D.getPosition().y * conversionFactor - yOffSet - turretYOffSet), AngleUnit.DEGREES, visionPose3D.getOrientation().getYaw());
    }
    ...
    public static class Output {
        ...
        public Output(Pose2D fusedPose, Pose2D visionPose, Pose2D pinpointPose, Pose2D filteredPose,
            ...
            this.filteredPose = filteredPose;
